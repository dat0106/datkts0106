  ...
  implements FragmentManager.BackStackEntry, Runnable
{
  ...
  void bumpBackStackNesting(int paramInt)
  {
    ...
    for (int i = -1 + localOp.removed.size();; i--)
    {
      ...
      if (FragmentManagerImpl.DEBUG) {
        Log.v("FragmentManager", "Bump nesting of " + localFragment2 + " to " + localFragment2.mBackStackNesting);
      }
    }
  }
      ...
      for (Op localOp = this.mHead;; localOp = localOp.next)
      {
        ...
        case 2: 
          ...
          for (int i = 0;; i++)
          {
            ...
            if (FragmentManagerImpl.DEBUG) {
              Log.v("FragmentManager", "OP_REPLACE: adding=" + localFragment2 + " old=" + localFragment1);
            }
